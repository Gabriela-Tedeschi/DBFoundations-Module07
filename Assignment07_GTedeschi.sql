...
-- 2021-05-21,G Tedeschi,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_GTedeschi')
	 Begin 
	  Alter Database [Assignment07DB_GTedeschi] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_GTedeschi;
	 End
	Create Database Assignment07DB_GTedeschi;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_GTedeschi;
...
SELECT
	ProductName
	,[InventoryDate] = (DateName(mm,InventoryDate)) + ', ' + ltrim(str(Year(InventoryDate)))
	,[Count]
FROM vProducts AS p
...
AS
	...
	ORDER BY CategoryName, Format(InventoryDate, 'd', 'en-US' );
GO
...
AS
	SELECT TOP 10000
		...
		,[PreviousMonthCount] = CASE InventoryDate
								WHEN 'January, 2017' THEN 0
								ELSE Lag(Count) OVER (ORDER BY ProductName)
								END
